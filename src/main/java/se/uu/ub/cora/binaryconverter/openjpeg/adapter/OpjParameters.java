...
/**
 * Used to add parameters to use when running opj-commands.
 * 
 * Note: When using opj_decompress only inputPath, outputPath and numberOfThreads is to be used
 * among these commands.
 */
public interface OpjParameters {
	 ...
	 * Get input path
	 * 
	 * @return The input path opj is set to use
	 */
	String getInputPath();
	 ...
	 * Get output path
	 * 
	 * @return The output path opj is set to use
	 */
	...
	/**
	 * Number of threads for opj to use. Setting this to 0 enables all available cores.<br>
	 * One way to calculate it would be: Runtime.getRuntime().availableProcessors() / 2
	 * 
	 * @param numOfThreads
	 *            The amount of threads the opj command should utilize.
	 */
	 ...
	 */
	void opjCommand(String command);
}
